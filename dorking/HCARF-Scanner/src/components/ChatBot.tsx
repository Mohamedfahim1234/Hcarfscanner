  ...
  const sendMessage = async (msg) => {
    ...
    try {
      ...
      if (res.status === 400 && data.error && data.error.includes('API key')) {
        setMessages((prev) => [...prev, { role: 'bot', content: 'OpenRouter API key missing. Please set it in your .env and restart the backend.' }]);
        setStatus('error');
      } else if (data.reply) {
        let fallbackMsg = '';
        if (data.fallback === 'openrouter') fallbackMsg = ' (OpenRouter fallback)';
        if (data.fallback === 'openai') fallbackMsg = ' (OpenAI fallback)';
        setMessages((prev) => [...prev, { role: 'bot', content: data.reply + fallbackMsg }]);
        setStatus(data.fallback ? 'fallback' : 'success');
      } else {
        setMessages((prev) => [...prev, { role: 'bot', content: 'Sorry, I could not answer that.' }]);
        setStatus('error');
      }
      setSuggestions([
        'Summarize my scan results.',
        ...
        <div className="fixed bottom-24 right-8 z-50 max-w-full animate-fade-in cursor-move">
          ...
          >
            <Card className="bg-transparent border-none shadow-none rounded-2xl h-full">
              ...
              {/* Status and Example prompts */}
              <div className="px-4 pb-3 pt-1 text-xs text-slate-400 flex flex-col gap-1">
                {loading && <span className="text-purple-400 animate-pulse">AI is typing…</span>}
                {status === 'fallback' && <span className="text-orange-400">Fallback model used due to primary failure.</span>}
                {status === 'error' && <span className="text-red-400">AI service unavailable or misconfigured.</span>}
